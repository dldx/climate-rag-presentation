...
>
	...
	>
		...
		>
			<div class=" m-16 grid grid-cols-{n_cols} items-stretch gap-3">
				<Transition order={1} enter="fade-enter" do={() => (n_cols = 1)}>
					<div class="h-full max-w-[30ch] border border-2 border-[#e03131] bg-[var(--r-background-color)] p-8 flex flex-col">
						<h2 class="mb-5 text-2xl">LLMs</h2>
						<ul class="list-disc pl-5 text-left text-xl">
							<li>Even the best LLMs occasionally hallucinate when sources don't contain enough information. More safeguards needed.</li>
							<li>Multi-modal LLMs are still not capable of parsing complex tables.</li>
							<li>LLMs cannot handle large tables of data natively. Use of SQL/pandas queries may help here.</li>
							<li>LLMs are not yet good at understanding complex themes referenced across large documents. <a href="https://microsoft.github.io/graphrag/">Graph-based</a> <a href="https://github.com/HKUDS/LightRAG">solutions</a> may help here.</li>
							<li>LLMs still have a very small output length, making them less suitable for compiling large tables</li>
						</ul>
						<img
							class="h-20 w-20 mt-auto mx-auto"
				...
				<Transition order={2} enter="fade-enter" do={() => (n_cols = 2)}>
					<div class="h-full max-w-[30ch] border border-2 border-[#e03131] bg-[var(--r-background-color)] p-8 flex flex-col">
						<h2 class="mb-5 text-2xl">Vector-based retrieval</h2>
						<ul class="list-disc pl-5 text-left text-xl">
							<li>Multi-lingual data requires multi-lingual embeddings model, which are often only strong in European languages (+ Chinese)</li>
							<li>Retrieving, eg., a Vietnamese document using an English query is less effective</li>
							<li>Vector databases are more expensive than traditional databases at scale</li>
							<li>Adopting improved embeddings models requires regenerating embeddings for entire database</li>
				...
				<Transition order={3} enter="fade-enter" do={() => (n_cols = 3)}>
					<div class="h-full max-w-[30ch] border border-2 border-[var(--r-climate-rag-color)] bg-[var(--r-background-color)] p-8 flex flex-col">
						<h2 class="mb-5 text-2xl">Climate-RAG pipeline</h2>
						<ul class="list-disc pl-5 text-left text-xl">
							<li>Data in charts and images are ignored (for now)</li>
							<li>Answering complex, multi-step questions requires manual orchestration (for now)</li>
							<li>User decides whether to do a web search or not, rather than automatically inferring based on database contents</li>
							<li>Each query has a cost, which could be mitigated using local LLMs</li>
							<li>If user doesn't use sensible database filters, LLM can confuse information about two companies</li>
							<li>For questions where the source of truth is known and in a structured format, classical data science techniques are more reliable.</li>
						</ul>
						<img
							class="h-20 w-20 mt-auto mx-auto"
							alt="Vector database icon"
